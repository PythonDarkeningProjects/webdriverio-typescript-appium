...
export class NavigationHandler {
    ...
    public timeEntryContinueButton: ElementWrapper;
    public timeEntryErrorLabel: ElementWrapper;
    public timeEntryNoteInput: ElementWrapper;
    ...
    public async employeeIDScreen() {
        ...
        this.employeeIDInput = this.profile.localeCompare('profile:iOS') === 0 ? employeeIDScreen.iosInput : employeeIDScreen.androidInput;
    }
    ...
    public async selectCompanyScreen() {
        ...
        if (generalSettings.isLocalDbEnvironment) {
            await this.useManualSetup();
        } else {
            await this.useCustomerID();
        }
        ...
        this.selectCompanyBackButton = this.profile.localeCompare('profile:iOS') === 0 ? selectCompanyScreen.iosBackButton : selectCompanyScreen.androidBackButton;
        this.selectCompanyList = this.profile.localeCompare('profile:iOS') === 0 ? selectCompanyScreen.iosList : selectCompanyScreen.androidList;
        this.selectCompanyScreenList = await elementHandler.findElementsFromDomUsingXpath(this.selectCompanyList);
        await elementHandler.waitForElementAppears(this.selectCompanyBackButton);
    }
    ...
    public async serverInformationScreen() {
        ...
        this.hostInput = this.profile.localeCompare('profile:iOS') === 0 ? serverInformationScreen.iosHostInput : serverInformationScreen.androidHostInput;
        this.portInput = this.profile.localeCompare('profile:iOS') === 0 ? serverInformationScreen.iosPortInput : serverInformationScreen.androidPortInput;
        this.alert = this.profile.localeCompare('profile:iOS') === 0 ? alerts.iosAlert : alerts.androidAlert;
    ...
    private getCompanyName() {
        return (this.companyName === undefined) ? baseScreen.defaultCompanyName : this.companyName;
    }

    private getEmployeeID() {
        return (this.employeeID === undefined) ? baseScreen.defaultEmployeeID : this.employeeID;
    }

    private getNamespace() {
        return (this.namespace === undefined) ? baseScreen.defaultNamespace : this.namespace;
    }

    private getPort() {
        return (this.port === undefined) ? generalSettings.port : this.port;
    }

    private async isSelectCompanyScreenDisplayed() {
    ...
    private async useCustomerID() {
        await this.welcomeScreen();
        let customerID: string = generalSettings.customerID ? generalSettings.customerID : 'iTexicoSandbox';
        await elementHandler.elementSendKeys(this.customerIDInput, customerID);
        await elementHandler.hideKeyboard();
        await elementHandler.clickOnElement(welcomeScreen.nextButton);
    }

    private async useManualSetup() {
        await this.serverInformationScreen();

        let radioButton: ElementWrapper = generalSettings.isHTTP ? serverInformationScreen.httpRadioButton : serverInformationScreen.httpsRadioButton;
        let host: string = generalSettings.host ? generalSettings.host : this.host;
        let port: string = generalSettings.port ? generalSettings.port : this.getPort();

        await elementHandler.clickOnElement(radioButton);
        await elementHandler.elementSendKeys(this.hostInput, host);
        await elementHandler.elementSendKeys(this.portInput, port);
        await elementHandler.hideNumericKeyboard(serverInformationScreen.image);
        await elementHandler.clickOnElement(serverInformationScreen.nextButton);
        await elementHandler.waitForElementAppears(namespaceScreen.backButton);
        this.namespaceInput = this.profile.localeCompare('profile:iOS') === 0 ? namespaceScreen.iosInput : namespaceScreen.androidInput;
        await elementHandler.waitForElementAppears(this.namespaceInput);

        await elementHandler.elementSendKeys(this.namespaceInput, this.getNamespace());
        await elementHandler.hideKeyboard();
        await elementHandler.clickOnElement(namespaceScreen.nextButton);
    }
